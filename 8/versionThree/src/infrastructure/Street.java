...
import customlist.LDLinkedList;
...
public class Street {
	private int length;
	private LDLinkedList<Building> leftSide;
	private LDLinkedList<Building> rightSide;
	...
	public Street(int length) {
		this.length = length;
		this.leftSide = new LDLinkedList<Building>();
		this.rightSide = new LDLinkedList<Building>();
	...
	/**
	 * 
	 * @return returns a {@link LDLinkedList} of {@link Building}
	 */
	public LDLinkedList<Building> left() {
	...
	/**
	 * 
	 * @return returns @return returns a {@link LDLinkedList} of {@link Building}
	 */
	public LDLinkedList<Building> right() {
	...
	public Building remove(int buildingId,Side side) {
		if(side == Side.left) {
			...
			for(int i = 0; i < rightSide.size(); i++) {
				if(rightSide.get(i).getId() == buildingId) {
					Building temp = rightSide.get(i);
					leftSide.remove(i);
					return temp;
				}
			}
	 ...
	 * @param side is of type {@linkplain Side}
